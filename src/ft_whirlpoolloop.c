...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/10/25 04:50:01 by vkryvono          #+#    #+#             */
/*   Updated: 2018/10/25 05:09:14 by vkryvono         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static const unsigned long long	g_rc[10] =
{
	0x1823C6E887B8014F, 0x36A6D2F5796F9152, 0x60BC9B8EA30C7B35,
	0x1DE0D7C22E4BFE57, 0x157737E59FF04ADA, 0x58C9290AB1A06B85,
	0xBD5D10F4CB3E0567, 0xE427418BA77D95D8, 0xFBEE7C66DD17479E,
	0xCA2DBF07AD5A8333
};

static const unsigned long long	g_t[256] =
{
...
static unsigned long long		ft_whirlpoolrnd(unsigned long long *b,
												const unsigned long long *a,
												int n, unsigned long long c)
{
	*b = g_t[(a[n] >> 56) & 0xFF];
	*b ^= ROTR64(g_t[(a[(n + 7) % 8] >> 48) & 0xff], 8);
	*b ^= ROTR64(g_t[(a[(n + 6) % 8] >> 40) & 0xff], 16);
	*b ^= ROTR64(g_t[(a[(n + 5) % 8] >> 32) & 0xff], 24);
	*b ^= ROTR64(g_t[(a[(n + 4) % 8] >> 24) & 0xff], 32);
	*b ^= ROTR64(g_t[(a[(n + 3) % 8] >> 16) & 0xff], 40);
	*b ^= ROTR64(g_t[(a[(n + 2) % 8] >> 8) & 0xff], 48);
	*b ^= ROTR64(g_t[a[(n + 1) % 8] & 0xff], 56);
	*b ^= c;
	return (*b);
}
...
void							ft_whirlpoolloop(t_whirlpool *msg)
{
	...
	j = -1;
	while (++j < 8)
		msg->k[j] = msg->hash.h[j];
	while (--j >= 0)
		msg->word.x[j] = ft_btoh(msg->word.x[j], sizeof(msg->word.x[j]));
	while (++j < 8)
		msg->state[j] = msg->word.x[j] ^ msg->k[j];
	i = -1;
	while (++i < 10)
	{
		while (--j > 0)
			msg->l[j] = ft_whirlpoolrnd(&(msg->l[j]), msg->k, j, 0);
		ft_whirlpoolrnd(&(msg->l[j--]), msg->k, 0, g_rc[i]);
		while (++j < 8)
			msg->k[j] = msg->l[j];
		while (--j >= 0)
			msg->l[j] = ft_whirlpoolrnd(&(msg->l[j]), msg->state, j, msg->k[j]);
		while (++j < 8)
			msg->state[j] = msg->l[j];
	}
	while (--j >= 0)
		msg->hash.h[j] ^= msg->state[j] ^ msg->word.x[j];
}
